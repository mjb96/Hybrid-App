...
| AUD-18 | Active native GPS points are process-memory only. | `GpsPointStore.kt`; `GpsTrackingService.kt`. | Store lifecycle and `START_NOT_STICKY` inspection show no durable point journal/recovery. No OEM-kill test was available. | Probable user impact |
